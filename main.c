...
giko_unicode_str_t *new_art_str(giko_bitmap_t *reference, giko_glyph_map_t *map, double greed);
giko_match_t best_scanline_match(giko_bitmap_t *reference, giko_glyph_map_t *map, int x, int y, double greed);
giko_bitmap_t *crop_bitmap(giko_bitmap_t *bitmap, int offset_x, int offset_y, int width, int height);
...
giko_unicode_str_t *new_art_str(giko_bitmap_t *reference, giko_glyph_map_t *map, double greed) {
    ...
    for (int row = 0; row < rows; row++) {
        ...
        while (x < width) {
            int y = row * em_height;
            giko_match_t best_match = best_scanline_match(reference, map, x, y, greed);
...
giko_match_t best_scanline_match(giko_bitmap_t *reference, giko_glyph_map_t *map, int x, int y, double greed) {
    ...
    while (advance > 0 && best_match.similarity < greed) {
        ...
        giko_bitmap_t *patch = crop_bitmap(reference, x, y, advance, map->em_height);
